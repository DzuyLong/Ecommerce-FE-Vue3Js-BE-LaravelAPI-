<template>
  <header class="flex justify-between items-center p-3 h-14 shadow bg-white">
    <button @click="emit('toggle-sidebar')"
      ...
      <ArrowLeftIcon @click="addclass" :class="{ 'none': !isAddClass }" class="w-full p-1 transition-all" />
      <ArrowRightIcon @click="addclass" :class="{ 'none': isAddClass }" class="w-full p-1 transition-all" />
    </button>
...
<script setup>
import { ArrowLeftIcon, ArrowRightIcon, ArrowUturnRightIcon, ChevronDownIcon, UserIcon } from '@heroicons/vue/24/outline'
